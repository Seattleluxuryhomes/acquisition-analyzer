...
export async function buildShowcase({ listing, photos, opts = {}, onProgress = () => {} }) {
  ...
  if (videoToolingReady()) {
    try {
      ...
      await stitchShowcase({ clips: clips.map((c) => c.file), listing, dims: [w, h], workDir: runDir, out, clipDuration: duration, labels: photos.map((p) => p.label), cards: opts.cards || {}, includeDetails: opts.includeDetails !== false });
